{{-- resources/views/alumno/entregas/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('alumno.entregas.show', $entrega) }}"
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        <h4 class="text-sm font-semibold text-amber-900 mb-1">
                            Tu entrega está en estado:
                            <span class="uppercase">{{ $entrega->estado }}</span>
                        </h4>
                        @if($entrega->observaciones)
                            <p class="text-sm text-amber-800 mt-2">
                                <span class="font-medium">Observaciones del docente:</span><br>
                    ...
                    <div class="flex-1">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-sm text-gray-900 font-medium">{{ $entrega->actividad->semana->nombre }}</p>
                            </div>

                            <div>
                                <p class="text-xs font-medium text-gray-500 uppercase">Tipo</p>
                                <p class="text-sm text-gray-900 font-medium">{{ $entrega->actividad->tipoActividad->nombre }}</p>
                            </div>

                            <div>
                                <p class="text-xs font-medium text-gray-500 uppercase">Fecha Inicio</p>
                                <p class="text-sm text-gray-900 font-medium">{{ $entrega->actividad->fecha_inicio->format('d/m/Y H:i') }}</p>
                            </div>

                            <div>
                                <p class="text-xs font-medium text-gray-500 uppercase">Fecha Límite</p>
                                <p class="text-sm text-red-600 font-medium">{{ $entrega->actividad->fecha_limite->format('d/m/Y H:i') }}</p>
                            </div>
                        </div>

                        @if($entrega->actividad->descripcion)
                            <div class="mt-4 pt-4 border-t border-gray-200">
                                <p class="text-xs font-medium text-gray-500 uppercase mb-1">Descripción</p>
                                <p class="text-sm text-gray-700">{{ $entrega->actividad->descripcion }}</p>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
                <div class="flex items-start space-x-4">
                    <div class="flex-shrink-0 w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center">
                        @svg('heroicon-o-document', 'w-6 h-6 text-gray-600')
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Entrega Anterior</h3>

                        @php
                            // Verificar si es una URL (Google Drive)
                            $esDriveUrl = filter_var($entrega->ruta, FILTER_VALIDATE_URL);
                        @endphp
                        ...
                        @if($esDriveUrl)
                            ...
                            <div class="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">Archivo de Google Drive</p>
                        ...
                        @else
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 border border-gray-200 rounded-lg">
                                <div class="flex items-center space-x-3">
                                    @svg('heroicon-o-document', 'w-5 h-5 text-gray-600')
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">{{ basename($entrega->ruta) }}</p>
                        ...
                        <div class="mt-3 text-xs text-gray-500">
                            Entregado el: {{ $entrega->fecha_entrega->format('d/m/Y H:i') }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('alumno.entregas.update', $entrega) }}"
                  method="POST"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="bg-gradient-to-r from-indigo-50 to-purple-50 px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                            @svg('heroicon-o-arrow-path', 'w-5 h-5 mr-2 text-indigo-600')
                            Nueva Entrega
                        </h3>
                        <p class="text-sm text-gray-600 mt-1">
                            Modo de entrega:
                            <span class="font-semibold">
                                {{ $entrega->actividad->tipoActividad->modo_entrega === 'pdf' ? 'Archivo PDF' : 'Google Drive' }}
                            </span>
                        </p>
                    </div>
                        ...
                        @if($entrega->actividad->tipoActividad->modo_entrega === 'pdf')
                            ...
                            <div>
                                <label for="archivo" class="block text-sm font-medium text-gray-700 mb-2">
                                    Nuevo Archivo de Entrega <span class="text-red-500">*</span>
                                </label>
                            ...
                            @else
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        Nuevo Archivo de Google Drive <span class="text-red-500">*</span>
                                    </label>
                    ...
                    <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <button type="submit"
                                id="btnEntregar"
                                @if($entrega->actividad->tipoActividad->modo_entrega === 'drive' && !$driveConectado) disabled @endif
    ...
    @if($entrega->actividad->tipoActividad->modo_entrega === 'drive' && $driveConectado)
        {{-- Google Picker API --}}
